...
func RegisterRoutes(root *gin.Engine, v1 *gin.RouterGroup, authMiddleware gin.HandlerFunc, handler *Handler, limiter *appmiddleware.RateLimiter, rateCfg config.RateLimitConfig) {
	...
	chats.GET("/:conversationUuid/messages", appmiddleware.ValidateUUIDParams("conversationUuid"), handler.Messages)
	chats.POST("/:conversationUuid/messages", appmiddleware.ValidateUUIDParams("conversationUuid"), handler.SendMessage)
	chats.POST("/:conversationUuid/read", appmiddleware.ValidateUUIDParams("conversationUuid"), handler.MarkRead)

	messages := v1.Group("/messages")
	messages.Use(authMiddleware)
	messages.DELETE("/:messageUuid", appmiddleware.ValidateUUIDParams("messageUuid"), handler.DeleteMessage)
